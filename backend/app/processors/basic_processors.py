...
class CropProcessor(ImageProcessor):
    def __init__(self):
        super().__init__()
        self.name = "Crop"
        self.description = "Crop image to specified region"
        self.parameters = {
            "x": {
                "type": "number",
                "default": 0,
...
class DenoiseProcessor(ImageProcessor):
    ...
    def process(self, image_path, **kwargs):
        ...
        if method == 'fast':
            ...
            denoised = cv2.fastNlMeansDenoisingColored(image, None, strength, strength, 7, 35)
...
class FlatFieldCorrectionProcessor(ImageProcessor):
    ...
    def process_multi(self, images_dict, **kwargs):
        ...
        if projection is None or gain is None or dark is None:
            raise ValueError("Flat field correction requires projection, gain, and dark images")
...
class MultiplyProcessor(ImageProcessor):
    ...
    def process_multi(self, images_dict, **kwargs):
        ...
        result = (image1.astype(np.float32) / 255.0) * (image2.astype(np.float32) / 255.0)
