...
SUPPORTED_OUTPUT_FORMATS = {"png", "jpg", "jpeg", "bmp", "tiff", "tif", "webp"}
...
class PipelineExecutor:
    ...
    def execute(self, nodes: list[dict[str, Any]], edges: list[dict[str, Any]]):
        ...
        try:
            ...
            source_input_nodes = [node for node in nodes if node.get("type") == "input"]
            ...
            for input_node in source_input_nodes:
                ...
                if not file_id:
                    logger.warning(
                        "Input node %s has no file_id, skipping",
                        input_node.get("id"),
                    )
                    continue
                ...
                if not input_path:
                    logger.warning(
                        "Input image not found for node %s: %s",
                        input_node.get("id"),
                        file_id,
                    )
                    continue
                ...
                logger.info(
                    "Found input image for node %s at: %s",
                    input_node.get("id"),
                    input_path,
                )
                ...
                if test_image is None:
                    logger.warning(
                        "Cannot load input image for node %s: %s",
                        input_node.get("id"),
                        input_path,
                    )
                    continue
                ...
                logger.info(
                    "Input image loaded for node %s, shape: %s",
                    input_node.get("id"),
                    test_image.shape,
                )
            ...
            first_input_id = next(iter(node_outputs))
            processed_nodes = set()
            output_results = []

            # Process nodes in topological order
            execution_order = self._get_execution_order(execution_graph, first_input_id)
            ...
            for node_id in execution_order:
                ...
                if node["type"] == "input":
                    ...
                    if source_ext == ".npz":
                        ...
                        output_format = str(node.get("data", {}).get("format", "png")).lower()
                        if output_format not in SUPPORTED_OUTPUT_FORMATS:
                            logger.warning(
                                "Unsupported output format '%s' on node %s, defaulting to png",
                                output_format,
                                node.get("id"),
                            )
                            output_format = "png"
            ...
            if not output_results:
                ...
                for sink_node_id in sink_node_ids:
                    sink_node = nodes_by_id.get(sink_node_id)
                    if not sink_node:
                        continue
    ...
    def _build_execution_graph(
        ...
        for node in nodes:
            ...
            if not node_id:
                continue
            graph[node_id] = {"node": node, "inputs": [], "outputs": []}
    ...
    def _get_execution_order(self, graph: dict[str, Any], start_node_id: str):
        ...
        def visit(node_id):
            if node_id in visiting:
                raise ValueError(f"Cycle detected in pipeline graph at node: {node_id}")
    ...
    def _find_image_file(self, file_id: str, folder: str):
