    ...
    def execute(self, nodes, edges):
        ...
        try:
            ...
            for input_node in input_nodes:
                ...
                if not input_path:
                    logger.warning(f"Input image not found for node {input_node['id']}: {file_id}")
                    continue
                
                logger.info(f"Found input image for node {input_node['id']} at: {input_path}")
                
                # Verify the image can be loaded
                test_image = cv2.imread(input_path)
                if test_image is None:
                    logger.warning(f"Cannot load input image for node {input_node['id']}: {input_path}")
                    continue
                    
                logger.info(f"Input image loaded for node {input_node['id']}, shape: {test_image.shape}")
            ...
            execution_order = self._get_execution_order(execution_graph, first_input_node['id'])
            ...
            for node_id in execution_order:
                ...
                if node['type'] == 'input':
                    ...
                    output_path = os.path.join(outputs_folder, f"{output_id}.{output_format}")
                    ...
                    if hasattr(processor, 'multi_input') and processor.multi_input:
                        ...
                        logger.info(f"Multi-input node {node['id']}: mapping={input_mapping}")
                        ...
                        for slot_name, source_node_id in input_mapping.items():
                            ...
                            if source_image_path:
                                ...
                                if img is not None:
                                    ...
                                    logger.debug(f"  Mapped slot '{slot_name}' from node {source_node_id}")
                                else:
                                    logger.warning(f"  Could not load image for slot '{slot_name}' from {source_image_path}")
                            else:
                                logger.warning(f"  No output found for slot '{slot_name}' from node {source_node_id}")
                        ...
                        if len(images_dict) > 1:
                            sizes = {k: v.shape[:2] for k, v in images_dict.items()}
                            unique_sizes = set(sizes.values())
                            if len(unique_sizes) > 1:
                                # Use the first image's size as reference
                                ref_key = list(images_dict.keys())[0]
                                ref_h, ref_w = images_dict[ref_key].shape[:2]
                                logger.info(f"Resizing multi-input images to match {ref_key}: {ref_w}x{ref_h}")
                                for k, v in images_dict.items():
                                    if v.shape[:2] != (ref_h, ref_w):
                                        images_dict[k] = cv2.resize(v, (ref_w, ref_h), interpolation=cv2.INTER_LINEAR)
                    ...
                    outputs_folder = os.path.join(self.base_dir, 'outputs')
                    intermediate_path = os.path.join(outputs_folder, f"{intermediate_id}.png")
